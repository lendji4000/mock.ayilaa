// Countries stocke les informations relatives aux pays
const pays = getAllCountries()
function generateCountries () {
    const countries = []
    for (let id = 0; id < pays.length; id+=1) {
        const elt= pays[id]
        let subelt={}
        subelt.id=id+1;
        subelt.name=elt.translations.fr;
        subelt.alpha2Code=elt.alpha2Code;
        subelt.capital=elt.capital;
        subelt.population=elt.population;
        subelt.area=elt.area;
        subelt.languages=elt.languages;
        countries.push(subelt)
    }
    return countries
}
