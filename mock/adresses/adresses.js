...
function generateAddresses() {
  ...
  for (let id = 1; id <= 200; id += 1) {
    ...
    const randomCompleteCountry = countries.filter(
      (country) => country.id === randomCountryId
    )[0];
    ...
    const city = hoaxer.address.cityName;
    adr.id = id;
    adr.countryId = randomCountryId;
    adr.state = "";
    adr.zip = "";
    adr.neighborhood = generateRandomWord(10); // Quartier
    adr.adr_line1 = generateRandomWord(10);
    adr.adr_line2 = generateRandomWord(10);
